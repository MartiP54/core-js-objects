...
function mergeObjects(objects) {
  ...
  if (objects[0] && objects[1]) {
    Object.entries(temp[0]).forEach(([key, value]) => {
      Object.entries(temp[1]).forEach(([key1, value1]) => {
        if (key === key1) {
          temp[0][key] = value + value1;
          delete temp[1][key1];
        }
      });
    });
    result = Object.assign(temp[0], temp[1]);
  }
  if (objects[0] && !objects[1]) {
    result = Object.assign(temp[0]);
  }
  return result;
}
...
function compareObjects(obj1, obj2) {
  const tempstr1 = JSON.stringify(obj1);
  const tempstr2 = JSON.stringify(obj2);
...
function makeWord(lettersObject) {
  ...
  const tempArr = Array(templength).fill(0);
...
function sellTickets(queue) {
  ...
  return queue.length === 0 ? true : kassa - queue[queue.length - 1] >= 0;
}
